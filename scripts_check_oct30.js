...
(async () => {
  ...
  console.log(`October 30th entries (UTC range 2025-10-30): ${entries.length}\n`);
  ...
  // Also check what the calendar would see - entries that show as Oct 30 locally
  ...
  const oct30Local = allEntries.filter(e => {
    const d = new Date(e.createdAt);
    return d.getFullYear() === 2025 && d.getMonth() === 9 && d.getDate() === 30;
  });
  
  console.log(`\nEntries showing as October 30th in local time: ${oct30Local.length}`);
  oct30Local.forEach((e, i) => {
    const d = new Date(e.createdAt);
